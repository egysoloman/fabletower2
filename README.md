...
- **FOUR playable characters** — RUNNER (versatile netdiver); VECTOR, a
  ...
  payoff attacks.
- **352 playable cards** — StS card volume across per-character pools and three RUNNER build
  archetypes —
...
- **30 potions** on a 3-slot belt, droppable via a hover ✕ (rules text
  generated by the same effect interpreter as cards) — drops, shop stock,
  and event rewards.
- **Ascension 0–20** (win to unlock the next level, matching the full StS climb), each level adding a
...
- **3 acts + a secret Act 4, 80 enemies, 12 bosses** — every act rotates
  between three or more bosses, with summoner broods, phase-shifting bosses, an
