...
reg(R({ id: 'copperwire', name: 'Copper Wire', rarity: 'common', sym: '〜', desc: 'Earn 8% more credits; start combats with 2 Block.', hooks: { goldBonusPct: 8, combatStartBlock: 2 } }))
reg(R({ id: 'gelpack', name: 'Gel Pack', rarity: 'common', sym: '◌', desc: 'Raise Max HP by 3; heal 2 after each combat.', hooks: { maxHp: 3, afterCombatHeal: 2 } }))
reg(R({ id: 'rustplate', name: 'Rust Plate', rarity: 'common', sym: '▥', desc: 'Start each combat with 7 Block.', hooks: { combatStartBlock: 7 } }))
reg(R({ id: 'sparkplug', name: 'Spark Plug', rarity: 'common', sym: '✧', desc: '+1 Energy on your first turn… at the cost of 3 Max HP.', hooks: { firstTurnEnergy: 1, maxHp: -3 } }))
reg(R({ id: 'fiberweave', name: 'Fiber Weave', rarity: 'common', sym: '⌘', desc: 'Start combats with 1 Plating; rests restore 3 more HP.', hooks: { combatStatuses: { plating: 1 }, restBonus: 3 } }))
reg(R({ id: 'loosechange', name: 'Loose Change', rarity: 'common', sym: '¢', desc: 'Earn 12% more credits.', hooks: { goldBonusPct: 12 } }))
reg(R({ id: 'solarvisor', name: 'Solar Visor', rarity: 'common', sym: '◠', desc: 'Rest sites restore 10 additional HP.', hooks: { restBonus: 10 } }))
reg(R({ id: 'splintcast', name: 'Splint Cast', rarity: 'common', sym: '⌐', desc: 'Raise your Max HP by 6.', hooks: { maxHp: 6 } }))
reg(R({ id: 'stungrip', name: 'Stun Grip', rarity: 'common', sym: '⌔', desc: 'Start with 1 Thorns and 3 Block.', hooks: { combatStatuses: { thorns: 1 }, combatStartBlock: 3 } }))
reg(R({ id: 'flashcap', name: 'Flash Capacitor', rarity: 'common', sym: '⌅', desc: 'Draw 1 more on your first turn; start with 2 Block.', hooks: { firstTurnDraw: 1, combatStartBlock: 2 } }))
reg(R({ id: 'vialrack', name: 'Vial Rack', rarity: 'common', sym: '⫙', desc: 'Raise Max HP by 4; heal 1 after each combat.', hooks: { maxHp: 4, afterCombatHeal: 1 } }))
reg(R({ id: 'jumpleads', name: 'Jump Leads', rarity: 'common', sym: '⌇', desc: 'Gain 1 Energy on reshuffle… at the cost of 2 Max HP.', hooks: { onShuffleEnergy: 1, maxHp: -2 } }))
reg(R({ id: 'meshsocks', name: 'Mesh Socks', rarity: 'common', sym: '∾', desc: 'Rests restore 6 more HP; earn 4% more credits.', hooks: { restBonus: 6, goldBonusPct: 4 } }))
reg(R({ id: 'groundspike', name: 'Ground Spike', rarity: 'common', sym: '▽', desc: 'Start with 3 Thorns… at the cost of 4 Max HP.', hooks: { combatStatuses: { thorns: 3 }, maxHp: -4 } }))
reg(R({ id: 'ledvisor', name: 'LED Visor', rarity: 'common', sym: '⎚', desc: 'Draw 1 more on your first turn; rests restore 2 more HP.', hooks: { firstTurnDraw: 1, restBonus: 2 } }))
reg(R({ id: 'blastcase', name: 'Blast Case', rarity: 'common', sym: '⛒', desc: 'Start with 4 Block; earn 4% more credits.', hooks: { combatStartBlock: 4, goldBonusPct: 4 } }))
reg(R({ id: 'dynamocoil', name: 'Dynamo Coil', rarity: 'rare', sym: '⟴', desc: 'Gain 2 Energy whenever your draw pile is shuffled.', hooks: { onShuffleEnergy: 2 } }))
reg(R({ id: 'aegismatrix', name: 'Aegis Matrix', rarity: 'rare', sym: '⛊', desc: 'Start with 10 Block; block cards grant 1 more.', hooks: { combatStartBlock: 10, cardBlockBonus: 1 } }))
reg(R({ id: 'hunterlens', name: 'Hunter Lens', rarity: 'rare', sym: '⊚', desc: 'Enemies start combat with 2 Vulnerable.', hooks: { combatStartEnemyStatuses: { vuln: 2 } } }))
reg(R({ id: 'leechrig', name: 'Leech Rig', rarity: 'rare', sym: '⥊', desc: 'Heal 8 after each combat; earn 5% more credits.', hooks: { afterCombatHeal: 8, goldBonusPct: 5 } }))
reg(R({ id: 'warspool', name: 'War Spool', rarity: 'rare', sym: '⍟', desc: 'Start with 1 Strength and 5 Block.', hooks: { combatStatuses: { str: 1 }, combatStartBlock: 5 } }))
reg(R({ id: 'focusarray', name: 'Focus Array', rarity: 'rare', sym: '⁘', desc: 'First turn: +1 Energy and draw 2 more cards.', hooks: { firstTurnDraw: 2, firstTurnEnergy: 1 } }))
reg(R({ id: 'blooddrive', name: 'Blood Drive', rarity: 'rare', sym: '⛾', desc: 'Raise Max HP by 12; heal 4 after each combat.', hooks: { maxHp: 12, afterCombatHeal: 4 } }))
reg(R({ id: 'stormcell', name: 'Storm Cell', rarity: 'rare', sym: '≶', desc: 'Start with 3 Turret… at the cost of 5 Max HP.', hooks: { combatStatuses: { turret: 3 }, maxHp: -5 } }))
reg(R({ id: 'hardhat', name: 'Hard Hat', rarity: 'rare', sym: '⌓', desc: 'Start each combat with 3 Plating.', hooks: { combatStatuses: { plating: 3 } } }))
reg(R({ id: 'contagioncore', name: 'Contagion Core', rarity: 'rare', sym: '⌬', desc: 'Start with 1 Viral; your Corrupt lands 1 harder.', hooks: { combatStatuses: { viral: 1 }, corruptBonus: 1 } }))
reg(R({ id: 'karambit', name: 'Karambit Chip', rarity: 'rare', sym: '⋗', desc: 'Start with 2 Strength… at the cost of 8 Max HP.', hooks: { combatStatuses: { str: 2 }, maxHp: -8 } }))
reg(R({ id: 'luckyseven', name: 'Lucky Seven', rarity: 'rare', sym: '⁊', desc: 'Earn 30% more credits… at the cost of 7 Max HP.', hooks: { goldBonusPct: 30, maxHp: -7 } }))
reg(R({ id: 'archivelens', name: 'Archive Lens', rarity: 'rare', sym: '⌕', desc: 'Draw 2 more on your first turn; earn 8% more credits.', hooks: { firstTurnDraw: 2, goldBonusPct: 8 } }))
reg(R({ id: 'nulldrive', name: 'Null Drive', rarity: 'rare', sym: '◇', desc: 'Start with 1 Artifact… at the cost of 6 Max HP.', hooks: { combatStatuses: { artifact: 1 }, maxHp: -6 } }))
reg(R({ id: 'singularitydrive', name: 'Singularity Drive', rarity: 'boss', sym: '⊙', desc: '+1 Energy every turn… at the cost of 15 Max HP.', hooks: { energyPerTurn: 1, maxHp: -15 } }))
reg(R({ id: 'hivemindcap', name: 'Hivemind Cap', rarity: 'boss', sym: '⌂', desc: 'Draw 1 more every turn… but earn 20% fewer credits.', hooks: { drawPerTurn: 1, goldBonusPct: -20 } }))
reg(R({ id: 'paradoxclock', name: 'Paradox Clock', rarity: 'boss', sym: '↻', desc: 'First turn each combat: +2 Energy and draw 2 more.', hooks: { firstTurnEnergy: 2, firstTurnDraw: 2 } }))
reg(R({ id: 'titanreactor', name: 'Titan Reactor', rarity: 'boss', sym: '⏣', desc: 'Start each combat with 2 Strength and 2 Plating.', hooks: { combatStatuses: { str: 2, plating: 2 } } }))
reg(R({ id: 'eventhorizon', name: 'Event Horizon', rarity: 'boss', sym: '⍜', desc: 'Start each combat with 2 Artifact.', hooks: { combatStatuses: { artifact: 2 } } }))
reg(R({ id: 'burnbook', name: 'Burn Book', rarity: 'rare', sym: '≜', char: 'vector', desc: 'Start with 3 Heat and 2 Coolant.', hooks: { combatStatuses: { heat: 3, coolant: 2 } } }))
reg(R({ id: 'stagehook', name: 'Stage Hook', rarity: 'rare', sym: '⌒', char: 'ghost', desc: 'Start each combat with 3 Stance Wall.', hooks: { combatStatuses: { stancewall: 3 } } }))
...
reg(R({ id: 'busfare', name: 'Bus Fare', rarity: 'common', sym: '⊐', desc: 'Earn 6% more credits; raise Max HP by 2.', hooks: { goldBonusPct: 6, maxHp: 2 } }))
reg(R({ id: 'wristlight', name: 'Wrist Light', rarity: 'common', sym: '⚲', desc: 'Draw 1 additional card on your first turn.', hooks: { firstTurnDraw: 1 } }))
reg(R({ id: 'kevlarscarf', name: 'Kevlar Scarf', rarity: 'common', sym: '≈', desc: 'Raise Max HP by 5; start combats with 2 Block.', hooks: { maxHp: 5, combatStartBlock: 2 } }))
reg(R({ id: 'tinnedluck', name: 'Tinned Luck', rarity: 'common', sym: '⌻', desc: 'Rests restore 4 more HP; heal 1 after each combat.', hooks: { restBonus: 4, afterCombatHeal: 1 } }))
reg(R({ id: 'magcharger', name: 'Mag Charger', rarity: 'common', sym: '⌭', desc: 'Gain 1 Energy whenever your draw pile is shuffled.', hooks: { onShuffleEnergy: 1 } }))
reg(R({ id: 'gutterblade', name: 'Gutter Blade', rarity: 'common', sym: '⟀', desc: 'Start combats with 1 Strength… at the cost of 5 Max HP.', hooks: { combatStatuses: { str: 1 }, maxHp: -5 } }))
reg(R({ id: 'pigeoncoop', name: 'Pigeon Coop', rarity: 'common', sym: '⌤', desc: 'Earn 10% more credits; rests restore 2 more HP.', hooks: { goldBonusPct: 10, restBonus: 2 } }))
reg(R({ id: 'twinfan', name: 'Twin Fan', rarity: 'rare', sym: '⌗', desc: 'Gain 4 Block and 1 Strength whenever you reshuffle.', hooks: { onShuffleBlock: 4, onShuffleStr: 1 } }))
reg(R({ id: 'boneclamp', name: 'Bone Clamp', rarity: 'rare', sym: '⍚', desc: 'Raise Max HP by 10; start combats with 4 Block.', hooks: { maxHp: 10, combatStartBlock: 4 } }))
reg(R({ id: 'plasmalattice', name: 'Plasma Lattice', rarity: 'rare', sym: '⌗', desc: 'Block cards grant 2 more Block.', hooks: { cardBlockBonus: 2 } }))
reg(R({ id: 'signalbooster', name: 'Signal Booster', rarity: 'rare', sym: '⍙', desc: 'Enemies start with 1 Weak; you start with 1 Strength.', hooks: { combatStartEnemyStatuses: { weak: 1 }, combatStatuses: { str: 1 } } }))
reg(R({ id: 'osmoticskin', name: 'Osmotic Skin', rarity: 'rare', sym: '⏦', desc: 'Start combats with 2 Regen and 2 Block.', hooks: { combatStatuses: { regen: 2 }, combatStartBlock: 2 } }))
reg(R({ id: 'greywallet', name: 'Grey Wallet', rarity: 'rare', sym: '⌻', desc: 'Earn 20% more credits; rests restore 5 more HP.', hooks: { goldBonusPct: 20, restBonus: 5 } }))
reg(R({ id: 'vampcable', name: 'Vamp Cable', rarity: 'rare', sym: '⥅', desc: 'Heal 6 after each combat; raise Max HP by 6.', hooks: { afterCombatHeal: 6, maxHp: 6 } }))
reg(R({ id: 'shockabsorber', name: 'Shock Absorber', rarity: 'rare', sym: '⎍', desc: 'Start combats with 8 Block and 1 Plating.', hooks: { combatStartBlock: 8, combatStatuses: { plating: 1 } } }))
reg(R({ id: 'railcapacitor', name: 'Rail Capacitor', rarity: 'rare', sym: '⍢', desc: '+1 Energy on your first turn; gain 3 Block on reshuffle.', hooks: { firstTurnEnergy: 1, onShuffleBlock: 3 } }))
reg(R({ id: 'silentfan', name: 'Silent Fan', rarity: 'rare', sym: '⍉', desc: 'Whenever you play a Power, gain 6 Block; powers cost 0 less.', hooks: { onPowerBlock: 6 } }))
reg(R({ id: 'threadripper', name: 'Threadripper', rarity: 'rare', sym: '⌁', desc: 'Whenever you play a 0-cost card, gain 3 Block.', hooks: { zeroCostBlock: 3 } }))
reg(R({ id: 'plaguepin', name: 'Plague Pin', rarity: 'rare', sym: '⍭', desc: 'Enemies start with 3 Corrupt.', hooks: { combatStartEnemyStatuses: { corrupt: 3 } } }))
reg(R({ id: 'chromeliver', name: 'Chrome Liver', rarity: 'rare', sym: '⍥', desc: 'Raise Max HP by 8; earn 12% more credits.', hooks: { maxHp: 8, goldBonusPct: 12 } }))
reg(R({ id: 'nightclock', name: 'Night Clock', rarity: 'boss', sym: '◔', desc: 'Draw 1 additional card every turn… at the cost of 10 Max HP.', hooks: { drawPerTurn: 1, maxHp: -10 } }))
reg(R({ id: 'demoncore', name: 'Demon Core', rarity: 'boss', sym: '⏺', desc: 'Start combats with 3 Strength and 3 Thorns… at the cost of 12 Max HP.', hooks: { combatStatuses: { str: 3, thorns: 3 }, maxHp: -12 } }))
reg(R({ id: 'perpetualmotion', name: 'Perpetual Motion', rarity: 'boss', sym: '∞', desc: 'Gain 2 Energy and 4 Block whenever you reshuffle.', hooks: { onShuffleEnergy: 2, onShuffleBlock: 4 } }))
reg(R({ id: 'panopticon', name: 'Panopticon', rarity: 'boss', sym: '⊛', desc: 'Enemies start with 1 Weak, 1 Vulnerable and 2 Corrupt.', hooks: { combatStartEnemyStatuses: { weak: 1, vuln: 1, corrupt: 2 } } }))
reg(R({ id: 'midasloop', name: 'Midas Loop', rarity: 'boss', sym: '¤', desc: 'Earn 60% more credits… at the cost of 10 Max HP.', hooks: { goldBonusPct: 60, maxHp: -10 } }))
reg(R({ id: 'cinderbloom', name: 'Cinder Bloom', rarity: 'common', sym: '❁', char: 'vector', desc: 'Start combats with 2 Heat and 2 Block.', hooks: { combatStatuses: { heat: 2 }, combatStartBlock: 2 } }))
reg(R({ id: 'meltglove', name: 'Melt Glove', rarity: 'rare', sym: '⌖', char: 'vector', desc: 'Start with 1 Strength and 2 Heat.', hooks: { combatStatuses: { str: 1, heat: 2 } } }))
reg(R({ id: 'frostvalve', name: 'Frost Valve', rarity: 'common', sym: '❅', char: 'vector', desc: 'Start each combat with 3 Coolant.', hooks: { combatStatuses: { coolant: 3 } } }))
reg(R({ id: 'nightopera', name: 'Night Opera', rarity: 'rare', sym: '♩', char: 'ghost', desc: 'Start with 1 Tempo Loop and 3 Block.', hooks: { combatStatuses: { tempoloop: 1 }, combatStartBlock: 3 } }))
reg(R({ id: 'maskofmany', name: 'Mask of Many', rarity: 'common', sym: '☻', char: 'ghost', desc: 'Start with 1 Momentum and 2 Block.', hooks: { combatStatuses: { momentum: 1 }, combatStartBlock: 2 } }))
reg(R({ id: 'boltbucket', name: 'Bolt Bucket', rarity: 'common', sym: '⛏', char: 'array', desc: 'Start each combat with 1 Turret and 2 Block.', hooks: { combatStatuses: { turret: 1 }, combatStartBlock: 2 } }))
reg(R({ id: 'brooddock', name: 'Brood Dock', rarity: 'boss', sym: '⚓', char: 'array', desc: 'Start with 2 Focus… at the cost of 8 Max HP.', hooks: { combatStatuses: { focus: 2 }, maxHp: -8 } }))
reg(R({ id: 'wiretap', name: 'Wiretap', rarity: 'common', sym: '⌖', char: 'runner', desc: 'Enemies start with 1 Corrupt; earn 5% more credits.', hooks: { combatStartEnemyStatuses: { corrupt: 1 }, goldBonusPct: 5 } }))
reg(R({ id: 'exploitkit', name: 'Exploit Kit', rarity: 'rare', char: 'runner', sym: '⍯', desc: 'Enemies start with 2 Vulnerable… at the cost of 5 Max HP.', hooks: { combatStartEnemyStatuses: { vuln: 2 }, maxHp: -5 } }))
reg(R({ id: 'greenroom', name: 'Green Room', rarity: 'common', sym: '⌂', char: 'ghost', desc: 'Rests restore 8 more HP.', hooks: { restBonus: 8 } }))
reg(R({ id: 'furnacefaith', name: 'Furnace Faith', rarity: 'boss', sym: '♨', char: 'vector', desc: 'Start with 2 Ignition and 4 Coolant.', hooks: { combatStatuses: { ignition: 2, coolant: 4 } } }))
